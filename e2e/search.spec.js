...
test.describe('Search Functionality', () => {
        ...
        if (await searchInput.isVisible()) {
            ...
            if (countBeforeSearch > 0) {
                ...
                if (countAfterSearch > 0) {
                    ...
                    expect(countAfterSearch < countBeforeSearch ||
                        firstServiceText.toLowerCase().includes('clean')).toBeTruthy();
                }
            }
        }
    });
    ...
    test('should show "no results" when search has no matches', async ({ page }) => {
        ...
        if (await searchInput.isVisible()) {
            ...
            await expect(page.getByText(/no results|nothing found|couldn't find/i)).toBeVisible();
        }
    });
        ...
        if (await searchInput.isVisible()) {
            ...
            if (await suggestionElements.isVisible()) {
                ...
                if (await suggestions.count() > 0) {
                    ...
                    expect(firstSuggestionText.toLowerCase().includes('cle')).toBeTruthy();
