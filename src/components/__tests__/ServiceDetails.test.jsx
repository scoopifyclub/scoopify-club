...
jest.mock('@/components/ui/button', () => ({
    Button: ({ children, onClick, disabled }) => (<button onClick={onClick} disabled={disabled} data-testid="button">
      {children}
    </button>),
}));
describe('ServiceDetails Component', () => {
    ...
    test('renders service details when service is provided', () => {
        render(<ServiceDetails service={mockService} onClose={mockOnClose} onClaim={mockOnClaim}/>);
    ...
    test('does not render when service is null', () => {
        render(<ServiceDetails service={null} onClose={mockOnClose} onClaim={mockOnClaim}/>);
    ...
    test('shows arrive button when status is CLAIMED and onArrive is provided', () => {
        const claimedService = Object.assign(Object.assign({}, mockService), { status: 'CLAIMED' });
        render(<ServiceDetails service={claimedService} onClose={mockOnClose} onArrive={mockOnArrive}/>);
    ...
    test('shows complete button when status is ARRIVED and onComplete is provided', () => {
        const arrivedService = Object.assign(Object.assign({}, mockService), { status: 'ARRIVED' });
        render(<ServiceDetails service={arrivedService} onClose={mockOnClose} onComplete={mockOnComplete}/>);
    ...
    test('disables claim button when hasInProgressJob is true', () => {
        render(<ServiceDetails service={mockService} onClose={mockOnClose} onClaim={mockOnClaim} hasInProgressJob={true}/>);
