...
export default function PaymentBatchList() {
  ...
  const fetchBatches = async (page = 1) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/payments/batch?page=${page}&limit=${pagination.limit}`
      );
      if (!response.ok) {
  ...
  const handleDeleteBatch = async () => {
    try {
      const response = await fetch(
        `/api/admin/payments/batch/${batchToDelete.id}`,
        {
          method: "DELETE",
        }
      );
  ...
  const processBatch = async (batchId, paymentMethod) => {
    try {
      const response = await fetch(
        `/api/admin/payments/batch/${batchId}/process`,
        {
          method: "POST",
          headers: {
            ...
            {batches.map((batch) => (
              <TableRow key={batch.id}>
                <TableCell className="font-medium">
                  <Link
                    href={`/admin/payments/batches/${batch.id}`}
                ...
                <TableCell>
                  <Badge
                    className={`${
                      statusColors[batch.status] || "bg-gray-500"
                    }`}
                  >
                    {batch.status}
                  </Badge>
                ...
                <TableCell>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      <DropdownMenuItem
                        onClick={() =>
                          router.push(`/admin/payments/batches/${batch.id}`)
                        }
                      >
                      ...
                      {batch.status === "DRAFT" && (
                        <>
                          <DropdownMenuItem
                            onClick={() =>
                              router.push(`/admin/payments/batches/${batch.id}/edit`)
                            }
                          >
                          ...
                          {batch.paymentsCount > 0 && (
                            <DropdownMenuItem
                              onClick={() =>
                                router.push(`/admin/payments/batches/${batch.id}/process`)
                              }
                            >
