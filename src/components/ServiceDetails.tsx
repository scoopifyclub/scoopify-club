      ...
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle className="flex items-center justify-between">
            <span>{service.servicePlan?.name || 'Service Details'}</span>
            <Button
              variant="ghost"
              size="icon"
              onClick={onClose}
        ...
        <div className="space-y-6">
          ...
          <div className="flex space-x-2 pt-4">
            {service.status === 'SCHEDULED' && onClaim && (
              <Button 
                onClick={onClaim} 
                className="flex-1"
            ...
            {service.status === 'CLAIMED' && onArrive && (
              <Button onClick={onArrive} className="flex-1">
                Mark as Arrived
              </Button>
            )}
            {service.status === 'ARRIVED' && onComplete && (
