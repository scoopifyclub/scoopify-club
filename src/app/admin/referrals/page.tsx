...
export default function AdminReferralsPage() {
  ...
  const handleProcessPayment = async (amount: number, cashAppPaymentId: string) => {
    try {
  ...
  const stats = {
    ...
    totalPaid: referrals.reduce((sum, r) => 
      sum + r.payments.reduce((pSum, p) => pSum + (p.status === 'SUCCESS' ? p.amount : 0), 0)
    , 0)
  };
                          ...
                          {referral.status === 'ACTIVE' && (
                            <>
                              <button
                                ...
                                disabled={processingPayment === referral.id}
                                className="text-blue-600 hover:text-blue-900 disabled:text-gray-400"
                              >
                                {processingPayment === referral.id ? (
