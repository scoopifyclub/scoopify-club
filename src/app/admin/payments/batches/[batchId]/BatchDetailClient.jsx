...
export default function BatchDetailClient({ batchId }) {
    var _a;
    ...
    const togglePaymentSelection = (paymentId) => {
        setSelectedPaymentsToAdd((prev) => prev.includes(paymentId)
    ...
    return (<div className="container py-6">
      <Breadcrumbs className="mb-6" items={[
            ...
            { href: `/admin/payments/batches/${batchId}`, label: (batch === null || batch === void 0 ? void 0 : batch.name) || `Batch #${batchId}` }
        ]}>
      </Breadcrumbs>
      ...
      <div className="grid gap-6">
        ...
        <Card>
          ...
          <CardContent>
            ...
            {/* Batch timeline */}
            {(batch.processingStartedAt || batch.completedAt) && (<div className="mt-6 border-t pt-4">
                ...
                <div className="space-y-2">
                  ...
                  {batch.processingStartedAt && (<div className="flex items-center text-sm">
                  ...
                  {batch.completedAt && (<div className="flex items-center text-sm">
            ...
            {batch.notes && (<div className="mt-6 border-t pt-4">
          ...
          <CardFooter className="border-t pt-4 flex flex-wrap gap-2">
            ...
            {batch.status === "DRAFT" && batch.paymentsCount > 0 && (<Button asChild className="ml-auto">
            ...
            {batch.status === "FAILED" && (<Button variant="destructive" asChild className="ml-auto">
                <Link href={`/admin/payments/batches/${batchId}/process`}>
                  <Play className="h-4 w-4 mr-2"/>
                  Retry Processing
                </Link>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              {isEditable && (<Button size="sm" onClick={() => {
          ...
          <CardContent>
            {isAddingPayments ? (<div className="space-y-4">
                ...
                setSelectedPaymentsToAdd([]);
                    ...
                    <Button size="sm" onClick={handleAddPayments} disabled={selectedPaymentsToAdd.length === 0}>
                ...
                {availablePayments.length === 0 ? (<div className="text-center py-8">
                    ...
                    <Table>
                      ...
                      <TableBody>
                        {availablePayments.map((payment) => (<TableRow key={payment.id} className={selectedPaymentsToAdd.includes(payment.id) ? "bg-primary/5" : ""} onClick={() => togglePaymentSelection(payment.id)} style={{ cursor: "pointer" }}>
                            <TableCell>
                              <input type="checkbox" checked={selectedPaymentsToAdd.includes(payment.id)} onChange={() => togglePaymentSelection(payment.id)} className="h-4 w-4"/>
                            </TableCell>
                            <TableCell>
              ...
              </div>) : payments.length === 0 ? (<div className="text-center py-8">
                <p className="text-muted-foreground mb-4">No payments in this batch</p>
                {isEditable && (<Button variant="outline" onClick={() => {
                    setIsAddingPayments(true);
                    fetchAvailablePayments();
                }}>
                    <PlusCircle className="h-4 w-4 mr-2"/>
                ...
                <TabsContent value="all">
                  <PaymentTable payments={payments} isEditable={isEditable} onRemove={handleRemovePayment}/>
                </TabsContent>
                
                <TabsContent value="earnings">
                  <PaymentTable payments={payments.filter(p => p.type === "EARNINGS")} isEditable={isEditable} onRemove={handleRemovePayment}/>
                </TabsContent>
                
                <TabsContent value="referrals">
                  <PaymentTable payments={payments.filter(p => p.type === "REFERRAL")} isEditable={isEditable} onRemove={handleRemovePayment}/>
                </TabsContent>
              </Tabs>)}
          </CardContent>
        </Card>
        ...
        <TableBody>
          {payments.map((payment) => (<TableRow key={payment.id}>
              ...
              {isEditable && (<TableCell>
                  <Button variant="ghost" size="icon" onClick={() => onRemove(payment.id)} aria-label="Remove payment">
                    <X className="h-4 w-4"/>
                  </Button>
                </TableCell>)}
            </TableRow>))}
