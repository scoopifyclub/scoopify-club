...
export default function AdminPaymentsPage() {
    ...
    const togglePaymentSelection = (paymentId) => {
        setSelectedPayments(prev => prev.includes(paymentId)
    ...
    return (<div className="container mx-auto px-4 py-8">
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {/* Batch Actions */}
        {selectedPayments.length > 0 && (<div className="bg-blue-50 p-4 rounded-lg mb-6">
            <div className="flex justify-between items-center">
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleBatchApprove} disabled={processingBatch} variant="outline" className="bg-blue-100">
                  Approve Selected
                </Button>
                <Button onClick={handleBatchProcess} disabled={processingBatch} variant="default">
                  Process Payments
                </Button>
              </div>
            </div>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {payments.map((payment) => (<tr key={payment.id}>
                  <td className="px-2 py-4">
                    {payment.status === 'PENDING' && (<input type="checkbox" checked={selectedPayments.includes(payment.id)} onChange={() => togglePaymentSelection(payment.id)}/>)}
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${payment.type === 'EARNINGS' ? 'bg-blue-100 text-blue-800' :
                payment.type === 'REFERRAL' ? 'bg-purple-100 text-purple-800' :
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {payment.preferredPaymentMethod ? (<span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${payment.preferredPaymentMethod === 'CASH_APP'
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    {payment.status === 'PENDING' ? (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-yellow-100 text-yellow-800">
                        Pending
                      </span>) : payment.status === 'APPROVED' ? (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-blue-100 text-blue-800">
                        Approved
                      </span>) : payment.status === 'PAID' ? (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800">
                        Paid
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    {payment.status === 'PENDING' && (<Button variant="outline" onClick={() => {
        ...
        {showPaymentModal && selectedPayment && (<div className="fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center">
