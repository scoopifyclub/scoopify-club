            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Popover>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !date && "text-muted-foreground", errors.date && "border-red-500")}>
                      <CalendarIcon className="mr-2 h-4 w-4"/>
                      {date ? format(date, 'PPP') : <span>Select a date</span>}
                    </Button>
                  </PopoverTrigger>
                  <PopoverContent className="w-auto p-0" align="start">
                    <Calendar mode="single" selected={date} onSelect={setDate} disabled={(date) => isBefore(date, addDays(new Date(), 0))} initialFocus/>
                  </PopoverContent>
                </Popover>
              ...
              <div className="space-y-2">
                ...
                <Select value={timeSlot} onValueChange={setTimeSlot}>
                  <SelectTrigger id="time" className={cn(errors.timeSlot && "border-red-500")}>
                    <SelectValue placeholder="Select a time">
                      {timeSlot ? (<div className="flex items-center">
                  ...
                  <SelectContent>
                    {TIME_SLOTS.map((slot) => (<SelectItem key={slot} value={slot}>
                        {getTimeDisplay(slot)}
                      </SelectItem>))}
                  </SelectContent>
                </Select>
                {errors.timeSlot && (<p className="text-sm text-red-500">Time is required</p>)}
              </div>
              ...
              <div className="space-y-2">
                ...
                <Textarea id="reason" placeholder="Enter reason for rescheduling" value={reason} onChange={(e) => setReason(e.target.value)}/>
              </div>
            </CardContent>
