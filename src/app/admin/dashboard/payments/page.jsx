...
export default function PaymentsPage() {
    ...
    const { data: session, status } = useSession({
        required: true,
        onUnauthenticated() {
            router.push('/auth/login?callbackUrl=/admin/dashboard/payments');
        },
    });
        ...
        <CardContent>
          <div className="flex flex-col md:flex-row gap-4 mb-4">
            <div className="relative flex-1">
              ...
              <Input placeholder="Search by customer, email, or invoice..." className="pl-9" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
            </div>
            <div className="flex gap-2">
              <Button variant="outline" className="flex items-center" onClick={() => setVisibleFilters(!visibleFilters)}>
          ...
          {visibleFilters && (<div className="flex flex-wrap gap-4 mb-4 p-4 bg-muted/20 rounded-md">
              <div className="flex flex-col gap-2">
                <span className="text-sm font-medium">Date Range</span>
                <DateRangePicker value={dateRange} onChange={(value) => {
                setDateRange({
          ...
          <div className="rounded-md border">
            <Table>
              ...
              <TableBody>
                {getSortedPayments().length > 0 ? (getSortedPayments().map((payment) => {
            var _a;
            return (<TableRow key={payment.id} className="cursor-pointer hover:bg-muted/50" onClick={() => router.push(`/admin/dashboard/payments/${payment.id}`)}>
                      <TableCell>
                      ...
                      <TableCell>{((_a = payment.service) === null || _a === void 0 ? void 0 : _a.name) || '—'}</TableCell>
                      ...
                      <TableCell className="text-right" onClick={(e) => e.stopPropagation()}>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            {payment.status === 'successful' && (<DropdownMenuItem onClick={() => router.push(`/admin/dashboard/payments/${payment.id}?refund=true`)}>
                                Process Refund
                              </DropdownMenuItem>)}
