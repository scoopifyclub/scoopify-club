    ...
    const handleRefund = async () => {
        ...
        try {
            ...
            setPayment(prev => {
                if (!prev)
                    return null;
                return Object.assign(Object.assign({}, prev), { status: 'refunded', refundAmount: refundAmount, refundReason: refundFormData.reason, refundDate: new Date().toISOString() });
            });
    ...
    return (<div className="p-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <div className="flex gap-2">
          {payment.receiptUrl && (<Button variant="outline" onClick={() => window.open(payment.receiptUrl, '_blank')}>
              <Download className="h-4 w-4 mr-2"/>
              Download Receipt
            </Button>)}
          {payment.status === 'successful' && (<Button variant="default" onClick={() => setIsRefundDialogOpen(true)}>
              Process Refund
            </Button>)}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
            ...
            <div className="space-y-4">
              <div className="p-3 border rounded-md">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {payment.status === 'successful' ? (<CheckCircle className="h-5 w-5 text-green-500"/>) : payment.status === 'failed' ? (<XCircle className="h-5 w-5 text-red-500"/>) : null}
                </div>
              </div>
              {payment.status === 'refunded' && payment.refundAmount && (<div className="p-3 border rounded-md">
                  <p className="text-xs text-muted-foreground">Refund Amount</p>
                  <p className="text-xl font-bold">${payment.refundAmount.toFixed(2)}</p>
                  {payment.refundDate && (<p className="text-xs text-muted-foreground mt-1">
              ...
              {payment.status === 'refunded' && payment.refundReason && (<div className="mt-4 p-3 bg-blue-50 rounded-md">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <Button variant="outline" className="w-full" onClick={() => router.push(`/admin/dashboard/customers/${payment.customer.id}`)}>
        ...
        <Card>
          ...
          <CardContent>
            {payment.service ? (<div className="space-y-4">
                <div className="flex items-start">
                ...
                {payment.service.employee && (<div className="flex items-start">
                    <User className="h-5 w-5 mr-3 text-muted-foreground"/>
                    <div>
                ...
                <Button variant="outline" className="w-full mt-4" onClick={() => { var _a; return router.push(`/admin/dashboard/services/${(_a = payment.service) === null || _a === void 0 ? void 0 : _a.id}`); }}>
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="refundAmount" type="number" className="pl-7" placeholder="0.00" value={refundFormData.amount} onChange={(e) => setRefundFormData(Object.assign(Object.assign({}, refundFormData), { amount: e.target.value }))} min="0.01" max={payment.amount} step="0.01"/>
              </div>
            ...
            <div className="space-y-2">
              ...
              <textarea id="refundReason" className="w-full min-h-[100px] px-3 py-2 border rounded-md focus:outline-none focus:ring-2 focus:ring-primary" value={refundFormData.reason} onChange={(e) => setRefundFormData(Object.assign(Object.assign({}, refundFormData), { reason: e.target.value }))} placeholder="Please provide a reason for this refund..."/>
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setIsRefundDialogOpen(false)}>Cancel</Button>
            <Button variant="destructive" onClick={handleRefund} disabled={isSubmitting || !refundFormData.amount || !refundFormData.reason}>
