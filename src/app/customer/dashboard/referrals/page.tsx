...
export default function CustomerReferralsPage() {
  ...
  const [cashAppName, setCashAppName] = useState<string | null>(null);
  ...
  const fetchReferralData = async () => {
    try {
      ...
      // Calculate stats
      const activeReferrals = referralsData.referrals.filter((r: Referral) => r.isActive).length;
      const totalEarned = paymentsData.payments.reduce((sum: number, p: ReferralPayment) => sum + p.amount, 0);
  ...
  const handleUpdateCashApp = async (newCashAppName: string) => {
    try {
