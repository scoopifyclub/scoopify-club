...
export async function GET(req) {
    try {
        ...
        const profileData = Object.assign(Object.assign({}, employee), { name: employee.user.name, email: employee.user.email, image: employee.user.image });
...
export async function PUT(request) {
    try {
        ...
        if (updateData.bio)
            employeeUpdateData.bio = updateData.bio;
        if (updateData.experienceYears !== undefined)
            employeeUpdateData.experienceYears = updateData.experienceYears;
        if (updateData.specialties)
            employeeUpdateData.specialties = updateData.specialties;
        if (updateData.certifications)
            employeeUpdateData.certifications = updateData.certifications;
        if (updateData.paymentInfo)
            employeeUpdateData.paymentInfo = updateData.paymentInfo;
        if (updateData.availabilityHours)
            employeeUpdateData.availabilityHours = updateData.availabilityHours;
        if (updateData.preferredServiceAreas)
            employeeUpdateData.preferredServiceAreas = updateData.preferredServiceAreas;
        // Payment related fields
        if (updateData.cashAppUsername !== undefined)
            employeeUpdateData.cashAppUsername = updateData.cashAppUsername;
        if (updateData.stripeAccountId !== undefined)
            employeeUpdateData.stripeAccountId = updateData.stripeAccountId;
        if (updateData.preferredPaymentMethod !== undefined)
            employeeUpdateData.preferredPaymentMethod = updateData.preferredPaymentMethod;
        ...
        const updatedProfileData = Object.assign(Object.assign({}, updatedEmployee), { name: updatedUser.name, email: updatedUser.email, image: updatedUser.image });
