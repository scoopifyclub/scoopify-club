...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (isBefore(now, sevenAM) || isAfter(now, sevenPM)) {
            return NextResponse.json({ error: 'Services can only be claimed between 7:00 AM and 7:00 PM' }, { status: 400 });
        }
        ...
        if (employee.services.length > 0) {
            return NextResponse.json({ error: 'You already have an active service. Complete it before claiming another.' }, { status: 400 });
        }
        const { id } = params;
        ...
        const customerZipCode = (_b = service.customer.address) === null || _b === void 0 ? void 0 : _b.zipCode;
        if (!customerZipCode) {
            return NextResponse.json({ error: 'Customer address information is incomplete' }, { status: 400 });
        }
        const isInServiceArea = employee.serviceAreas.some(area => area.zipCode === customerZipCode);
        if (!isInServiceArea) {
            return NextResponse.json({ error: 'This service is outside your service area' }, { status: 400 });
        }
        ...
        try {
            await sendServiceNotificationEmail(updatedService.customer.user.email, updatedService.id, 'claimed', {
                date: updatedService.scheduledDate.toLocaleDateString(),
                address: `${((_c = updatedService.customer.address) === null || _c === void 0 ? void 0 : _c.street) || ''}, ${((_d = updatedService.customer.address) === null || _d === void 0 ? void 0 : _d.city) || ''}`,
