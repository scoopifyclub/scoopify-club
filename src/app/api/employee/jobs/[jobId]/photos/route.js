...
export async function POST(request, { params }) {
    ...
    try {
        ...
        if (service.status !== 'IN_PROGRESS') {
            return NextResponse.json({ error: 'Service must be in progress to upload photos' }, { status: 400 });
        }
        ...
        if (beforePhotos.length < 4 || afterPhotos.length < 4) {
            return NextResponse.json({ error: 'At least 4 before and 4 after photos are required' }, { status: 400 });
        }
        // Start a transaction
        const result = await prisma.$transaction(async (tx) => {
            // Save photos
            const savedPhotos = await Promise.all(photos.map(photo => tx.servicePhoto.create({
                data: {
                    serviceId: jobId,
                    url: photo.url,
                    type: photo.type,
            ...
            if (checklistItems && Array.isArray(checklistItems)) {
                await Promise.all(checklistItems.map(item => tx.serviceChecklist.create({
                    data: {
                        serviceId: jobId,
                        item: item.text,
