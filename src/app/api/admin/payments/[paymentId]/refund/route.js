...
export async function POST(request, { params }) {
    ...
    try {
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await logPaymentEvent(paymentId, 'PAYMENT_REFUNDED', {
                amount,
                reason,
                adminId: userId,
                refundTransactionId
        ...
        return NextResponse.json({ error: error instanceof Error ? error.message : 'Failed to process refund' }, { status: error instanceof Error && error.message.includes('not found') ? 404 : 500 });
    }
}
