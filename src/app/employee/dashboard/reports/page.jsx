...
import { Download, CalendarRange, TrendingUp, Star, Clock, Users, DollarSign, ThumbsUp, Award, CheckCircle2 } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="flex items-center">
            <div className="flex">
              {[1, 2, 3, 4, 5].map((star) => (<Star key={star} className={`h-4 w-4 ${star <= Math.round(performanceData.avgRating)
      ...
      <Tabs defaultValue="overview" className="mt-6">
            ...
            <Card>
              ...
              <CardContent className="flex justify-center">
                <div className="h-80 w-full">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={serviceTypeData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
        ...
        <TabsContent value="feedback" className="mt-4">
          <Card>
            ...
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="h-80">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={customerReviewsData} cx="50%" cy="50%" labelLine={false} outerRadius={80} fill="#8884d8" dataKey="value" label={({ name, percent }) => `${name}: ${(percent * 100).toFixed(0)}%`}>
                        {customerReviewsData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[index % COLORS.length]}/>))}
                      </Pie>
                      <Tooltip formatter={(value) => `${value}%`}/>
                      <Legend />
                    </PieChart>
            ...
            return (<div key={rating} className="flex items-center text-sm">
                            <span className="w-8 mr-2">{rating} ★</span>
                            <div className="flex-1 h-2 rounded-full bg-gray-200 overflow-hidden">
                              <div className="h-full rounded-full bg-amber-500" style={{ width: `${percentage}%` }}></div>
                            </div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <div className="space-y-3">
                      <div>
                        <div className="flex items-center">
                          <div className="flex">
                            {Array(5).fill(0).map((_, i) => (<Star key={i} className="h-3 w-3 text-amber-500 fill-amber-500"/>))}
                          </div>
