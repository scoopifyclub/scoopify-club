...
export default function ServicePage() {
    ...
    const handleArrival = async () => {
        try {
            const response = await fetch(`/api/services/${id}/arrive`, {
                method: 'POST',
            });
            if (!response.ok)
      ...
      <div className="bg-white rounded-lg shadow p-6">
        ...
        {arrived && (<>
            <div className="mb-6">
              ...
              <ServicePhotoUpload serviceId={id} type="BEFORE" onUploadComplete={() => toast.success('Before photos uploaded')}/>
            </div>

            <div className="mb-6">
              <h2 className="text-xl font-semibold mb-4">Service Checklist</h2>
              <ServiceChecklist serviceId={id} initialChecklist={service.checklist} onComplete={setChecklistComplete}/>
            </div>

            {checklistComplete && (<div className="mb-6">
                <h2 className="text-xl font-semibold mb-4">After Photos</h2>
                <ServicePhotoUpload serviceId={id} type="AFTER" onUploadComplete={() => toast.success('After photos uploaded')}/>
              </div>)}
