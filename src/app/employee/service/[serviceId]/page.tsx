...
export default function ServicePage({ params }: { params: { serviceId: string } }) {
  ...
  const [postCleanPhotos, setPostCleanPhotos] = useState<string[]>([]);
  ...
  const fetchService = async () => {
    try {
      ...
      const prePhotos = data.photos.filter((p: any) => p.type === 'PRE_CLEAN').map((p: any) => p.url);
      const postPhotos = data.photos.filter((p: any) => p.type === 'POST_CLEAN').map((p: any) => p.url);
  ...
  const handleArrival = async () => {
    try {
      ...
      const response = await fetch(`/api/employee/services/${params.serviceId}/arrive`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
  ...
  const handlePhotoUpload = async (files: FileList, type: 'PRE_CLEAN' | 'POST_CLEAN') => {
    try {
      ...
      for (const url of urls) {
        const response = await fetch(`/api/services/${params.serviceId}/photos`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('token')}`,
            'Content-Type': 'application/json'
  ...
  const handleChecklistUpdate = async (updates: Partial<typeof checklist>) => {
    try {
      const response = await fetch(`/api/services/${params.serviceId}/checklist`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
        },
        body: JSON.stringify({ ...checklist, ...updates })
      });
  ...
  const handleComplete = async () => {
    try {
      const response = await fetch(`/api/employee/services/${params.serviceId}/complete`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
  ...
  if (!service) return <div className="flex justify-center items-center min-h-screen">Service not found</div>;
  ...
  return (
    <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-4">
          ...
          <div className="flex space-x-2">
            <button
              ...
              className="flex items-center space-x-1 bg-blue-100 text-blue-800 px-3 py-1 rounded"
            >
      ...
      {showMap && (
        <div className="bg-white rounded-lg shadow-md p-6 mb-8">
          ...
          <div className="h-96 w-full">
            <Map
              address={`${service.customer.address.street}, ${service.customer.address.city}, ${service.customer.address.state} ${service.customer.address.zipCode}`}
            />
          </div>
        </div>
      )}
      ...
      <div className="space-y-8">
        ...
        {service.status === 'ARRIVED' && (
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              {preCleanPhotos.map((url, index) => (
                <img
                  key={index}
                  src={url}
                  alt={`Pre-clean photo ${index + 1}`}
                  className="w-full h-48 object-cover rounded"
                />
              ))}
            </div>
            {preCleanPhotos.length < 4 && (
              <div className="flex items-center justify-center w-full">
                <label className="w-full flex flex-col items-center px-4 py-6 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-100">
                  ...
                  <input
                    ...
                    accept="image/*"
                    multiple
                    onChange={(e) => e.target.files && handlePhotoUpload(e.target.files, 'PRE_CLEAN')}
                  />
                </label>
              </div>
            )}
        ...
        {/* Checklist Section */}
        {preCleanPhotos.length >= 4 && service.status !== 'COMPLETED' && (
          <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {Object.values(checklist).every(Boolean) && service.status !== 'COMPLETED' && (
          <div className="bg-white rounded-lg shadow-md p-6">
            <h2 className="text-xl font-semibold mb-4">Post-Clean Photos</h2>
            <div className="grid grid-cols-2 gap-4 mb-4">
              {postCleanPhotos.map((url, index) => (
                <img
                  key={index}
                  src={url}
                  alt={`Post-clean photo ${index + 1}`}
                  className="w-full h-48 object-cover rounded"
                />
              ))}
            </div>
            {postCleanPhotos.length < 4 && (
              <div className="flex items-center justify-center w-full">
                <label className="w-full flex flex-col items-center px-4 py-6 bg-gray-50 rounded-lg border-2 border-gray-300 border-dashed cursor-pointer hover:bg-gray-100">
                  <CameraIcon className="h-12 w-12 text-gray-400" />
                  <p className="mt-2 text-sm text-gray-500">Take photos (at least 4 required)</p>
                  <input
                    ...
                    accept="image/*"
                    multiple
                    onChange={(e) => e.target.files && handlePhotoUpload(e.target.files, 'POST_CLEAN')}
                  />
                </label>
              </div>
            )}
        ...
        {/* Complete Button */}
        {postCleanPhotos.length >= 4 && Object.values(checklist).every(Boolean) && service.status !== 'COMPLETED' && (
          <button
