export function checkTimeConflict(services, newService, currentUserId) {
    // Get all services assigned to the current user
    const userServices = services.filter(service => service.employeeId === currentUserId);
    ...
    const hasInProgressService = userServices.some(service => ['CLAIMED', 'ARRIVED', 'IN_PROGRESS'].includes(service.status));
    return hasInProgressService;
}
export function validateEmployeeAccess(service, currentUserId, isAdmin = false) {
...
export function validateServiceCompletion(service, currentUserId, isAdmin = false) {
...
export function validatePhotoUpload(service, currentUserId, isAdmin = false) {
    // Check employee access
    if (!validateEmployeeAccess(service, currentUserId, isAdmin)) {
        return {
            isValid: false,
...
export const validateServiceStatus = (service, newStatus, userId, isAdmin) => {
