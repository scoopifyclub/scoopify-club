 ...
 */
export async function reconcileStripePayments(startDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000), // Default: last 7 days
endDate = new Date()) {
 ...
 */
async function fetchStripePaymentIntents(startDate, endDate) {
 ...
 */
async function fetchStripeTransfers(startDate, endDate) {
    const transfers = [];
    let hasMore = true;
    let startingAfter = undefined;
    // Stripe uses unix timestamps in seconds
...
async function storeReconciliationResults(results) {
    ...
    for (const result of results) {
        if (result.matchStatus !== 'MATCHED' && result.paymentId) {
            ...
            await logPaymentEvent(result.paymentId, 'RECONCILIATION_ISSUE', {
 ...
 */
export async function getRecentReconciliationReports(limit = 10) {
 ...
 */
export async function getReconciliationReport(reportId) {
