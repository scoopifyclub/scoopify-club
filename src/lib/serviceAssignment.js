...
export async function findBestEmployee(serviceRequest) {
    ...
    const employeeScores = await Promise.all(employees.map(async (employee) => {
        ...
        const currentLocation = (lastService === null || lastService === void 0 ? void 0 : lastService.location)
            ? {
        ...
        const isPrimaryArea = employee.serviceAreas.some((area) => area.zipCode === serviceRequest.address.zipCode && area.isPrimary);
